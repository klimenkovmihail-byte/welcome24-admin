...
export default function CaseFinance({
  ...
  const isAdmin = ['super_admin', 'admin'].includes(getCurrentUser()?.role || '');

  // Загрузка/замена чека (PDF) — в редактируемой форме и в проведённой сделке (админ восстанавливает
  // потерянный чек). Бэк разрешает перезалив после проведения только админу.
  ...
  // Сделка уже проведена (после согласования) — итог без правок (но админ может восстановить чек).
  if (task.deal_id) {
    ...
    return (
      <Box sx={{ mt: 1, p: 1.2, borderRadius: 1.5, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)' }}>
        ...
        {(task.hasReceipt || isAdmin) && (
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mt: 0.5, flexWrap: 'wrap' }}>
            {task.hasReceipt && (
              <Button size="small" startIcon={<DescriptionRoundedIcon sx={{ fontSize: 16 }} />}
                onClick={() => openCaseReceipt(task.id).catch((err) => setError(err instanceof Error ? err.message : 'Не удалось открыть чек'))}
                sx={{ color: '#60A5FA', textTransform: 'none', fontSize: 12 }}>Открыть чек</Button>
            )}
            {isAdmin && (
              <Button component="label" size="small" disabled={uploading} sx={{ color: '#94A3B8', textTransform: 'none', fontSize: 12 }}>
                {uploading ? 'Загрузка…' : (task.hasReceipt ? 'заменить чек' : 'загрузить чек заново')}
                <input hidden type="file" accept="application/pdf,.pdf" onChange={onReceipt} />
              </Button>
            )}
          </Box>
        )}
  ...
  if (task.approval_status === 'pending') {
    return (
      <Box sx={{ mt: 1, p: 1.2, borderRadius: 1.5, background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.3)' }}>
        ...
        {task.hasReceipt && (
          <Button size="small" startIcon={<DescriptionRoundedIcon sx={{ fontSize: 16 }} />} onClick={() => openCaseReceipt(task.id).catch((err) => setError(err instanceof Error ? err.message : 'Не удалось открыть чек'))}
